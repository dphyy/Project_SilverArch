...
function render() {
  ...
  $("#report-app").innerHTML = `
    ...
    ${caseItem.audioUrl ? `<section class="report-audio"><audio id="report-audio" controls preload="metadata" src="${caseItem.audioUrl}"></audio><span>${formatTime(duration)} recording</span></section>` : '<div class="pending">This case has no attached audio.</div>'}
    <div class="report-layout"><article class="report-paper">
      ...
      ${report.reviewCouncil ? `<section><h2>Review council</h2><div class="council-summary ${report.reviewCouncil.humanEscalationRequired ? "escalate" : ""}"><strong>${report.reviewCouncil.humanEscalationRequired ? "Human escalation required" : "No escalation trigger detected"}</strong><span>${Number(report.reviewCouncil.confidenceScore || 0)}% confidence / readiness</span>${report.reviewCouncil.triggers?.length ? `<p>${escapeHtml(report.reviewCouncil.triggers.join(" · "))}</p>` : ""}</div><div class="council-grid">${(report.reviewCouncil.perspectives || []).map((item) => `<article class="council-card ${escapeHtml(item.status)}"><span>${escapeHtml(item.title)}</span><p>${escapeHtml(item.summary)}</p></article>`).join("")}</div></section>` : ""}
